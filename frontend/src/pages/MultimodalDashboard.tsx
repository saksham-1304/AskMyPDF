...
const MultimodalDashboard: React.FC = () => {
  ...
  const fetchDocuments = async () => {
    try {
      ...
      const response = await api.get(`/multimodal/documents?${params}`);
  ...
  const handleUploadComplete = async (documentId: string) => {
    ...
    setTimeout(() => {
      navigate(`/multimodal-chat/${documentId}`);
    }, 1000);
  };
  ...
  const handleDeleteDocument = async (id: string, event: React.MouseEvent) => {
    ...
    try {
      await api.delete(`/multimodal/documents/${id}`);
  ...
  const getModalityIcons = (modalities: string[]) => {
    ...
    return modalities.map(modality => iconMap[modality as keyof typeof iconMap]).filter(Boolean);
  };
        ...
        <div className="relative z-10 max-w-4xl mx-auto pt-8">
          ...
          >
            ...
            <p className="text-gray-600 dark:text-gray-400 text-lg">
              Upload PDF, DOCX, images, or audio files for intelligent conversations
            </p>
          </motion.div>
          
          <MultimodalFileUpload onUploadComplete={handleUploadComplete} />
        </div>
      </div>
    );
  }
      ...
      <div className="relative z-10 max-w-7xl mx-auto">
        ...
        >
          ...
          <p className="text-xl text-gray-600 dark:text-gray-400 mb-8">
            Chat with documents, images, and audio files using advanced AI
          </p>
        ...
        >
          {[
            ...
            },
            {
              ...
              value: documents.filter(doc => doc.fileType === 'text').length,
              color: "from-blue-500 to-cyan-500"
            },
            {
              icon: Image,
              title: "Images",
              value: documents.filter(doc => doc.fileType === 'image').length,
              color: "from-green-500 to-emerald-500"
            },
            {
              icon: Music,
              title: "Audio Files",
              value: documents.filter(doc => doc.fileType === 'audio').length,
          ...
          <GlassCard className="p-6">
            <div className="flex flex-col lg:flex-row gap-4">
              ...
              >
                ...
                <option value="image">Visual Content</option>
          ...
          {filteredDocuments.length === 0 ? (
            ...
            >
              <GlassCard className="p-12 max-w-md mx-auto" gradient>
                ...
                <h3 className="text-xl font-semibold text-gray-900 dark:text-white mb-2">
                  {documents.length === 0 ? 'No files yet' : 'No files match your search'}
                </h3>
                <p className="text-gray-600 dark:text-gray-400 mb-6">
                  {documents.length === 0 
                    ? 'Get started by uploading your first document, image, or audio file.' 
            ...
            >
              {filteredDocuments.map((document, index) => {
                ...
                const modalityIcons = getModalityIcons(document.metadata?.modalities || []);
                ...
                return (
                  ...
                  >
                    <GlassCard
                      ...
                      onClick={() => document.processingStatus === 'completed' && navigate(`/multimodal-chat/${document._id}`)}
                    ...
                    >
                        ...
                        <div className="flex items-center space-x-3 flex-1 min-w-0">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-sm text-gray-600 dark:text-gray-400">
                              {formatFileSize(document.size)} • {document.fileType}
                            </p>
                          </div>
                        </div>
                        <motion.button
                      ...
                      <div className="space-y-3">
                        {/* Modalities */}
                        {document.metadata?.modalities && document.metadata.modalities.length > 0 && (
                          <div className="flex items-center space-x-2">
                            <span className="text-xs text-gray-500 dark:text-gray-400">Modalities:</span>
                            <div className="flex space-x-1">
                              {modalityIcons.map((Icon, idx) => (
                                <Icon key={idx} className="h-3 w-3 text-gray-600 dark:text-gray-400" />
                              ))}
                            </div>
                          </div>
                        )}
                        ...
                        {document.metadata && (
                          <div className="text-xs text-gray-500 dark:text-gray-400 space-y-1">
                            {document.metadata.pages && (
                              <div>📄 {document.metadata.pages} pages</div>
                            )}
                            {document.metadata.wordCount && (
                              <div>📝 {document.metadata.wordCount} words</div>
                            )}
                            {document.metadata.duration && (
                              <div>⏱️ {Math.round(document.metadata.duration)}s duration</div>
                            )}
                            {document.metadata.width && document.metadata.height && (
                              <div>📐 {document.metadata.width}×{document.metadata.height}px</div>
                            )}
